...
# Forward functions
def q_sample(y, y_0_hat, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, noise=None):    # y_0_hat: (32, 240, 7), alphas_bar_sqrt, one_minus_alphas_bar_sqrt: (1000,), t (32,)
    """
    y_0_hat: prediction of pre-trained guidance model; can be extended to represent
        any prior mean setting at timestep T.
    """
    if noise is None:   # noise (32, 240, 7)
        noise = torch.randn_like(y).to(y.device)
    sqrt_alpha_bar_t = extract(alphas_bar_sqrt, t, y)   # (32, 1, 1)
    sqrt_one_minus_alpha_bar_t = extract(one_minus_alphas_bar_sqrt, t, y)   # (32, 1, 1)
    # q(y_t | y_0, x)
    # NOTE: use CARD diffusion (forward process)
...
def p_sample(model, x, x_mark, y, y_0_hat, y_T_mean, t, alphas, one_minus_alphas_bar_sqrt, use_multi_gpu=False):
    """
    ...
    if not use_multi_gpu:
        ...
        eps_theta = model(x, x_mark, 0, y, y_T_mean, t).to(device).detach()
    else:
...
def p_sample_t_1to0(model, x, x_mark, y, y_0_hat, y_T_mean, one_minus_alphas_bar_sqrt, use_multi_gpu=False):
    device = next(model.parameters()).device
    t = torch.tensor([0]).to(device)  # corresponding to timestep 1 (i.e., t=1 in diffusion models)
    sqrt_one_minus_alpha_bar_t = extract(one_minus_alphas_bar_sqrt, t, y)
    sqrt_alpha_bar_t = (1 - sqrt_one_minus_alpha_bar_t.square()).sqrt()
    if use_multi_gpu:
        # eps_theta = model(x, x_mark, 0, y, y_0_hat, t.repeat(x.shape[0], 1)).to(device).detach()
        eps_theta = model(x, x_mark, 0, y, y_T_mean, t.repeat(x.shape[0], 1)).to(device).detach()
    else:
        # eps_theta = model(x, x_mark, 0, y, y_0_hat, t).to(device).detach()
        eps_theta = model(x, x_mark, 0, y, y_T_mean, t).to(device).detach()
...
def p_sample_loop(model, x, x_mark, y_0_hat, y_T_mean, n_steps, alphas, one_minus_alphas_bar_sqrt, use_multi_gpu=False):
    device = next(model.parameters()).device
