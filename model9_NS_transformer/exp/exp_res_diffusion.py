...
class Exp_Main_ResDiffusion(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:   # false
                    ...
                    if self.args.output_attention:  # false
                        ...
                        output = self.model(batch_x, batch_x_mark, res_batch_y, y_t_batch, y_T_mean, t)    # epsilon theta (32, 240, 7)
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:   # false
                    ...
                    if self.args.output_attention:  # false
                        ...
                        output = self.model(batch_x, batch_x_mark, res_batch_y, y_t_batch, y_T_mean, t)    # (32, 240, 7)
    ...
    def test(self, setting, test=0):
        ...
        def store_gen_y_at_step_t(config, config_diff, idx, y_tile_seq):
            ...
            """
            current_t = get_attribute('diffusion_steps') - idx
        ...
        print('NT metrc: mse:{:.4f}, mae:{:.4f} , rmse:{:.4f}, mape:{:.4f}, mspe:{:.4f}'.format(mse, mae, rmse, mape,
                                                                                                mspe))
        preds = preds.reshape(-1, preds.shape[-3], preds.shape[-2] * preds.shape[-1])   # (1, 3288, 134400)
        preds = preds.transpose(0, 2, 1)    # (3288, 1344, 100)
        preds = preds.reshape(-1, preds.shape[-1])      # (4419072, 100)

        trues = trues.reshape(-1, 1, trues.shape[-2] * trues.shape[-1])     # (3288, 1, 1344)
        trues = trues.transpose(0, 2, 1)    # (3288, 1344, 1)
        trues = trues.reshape(-1, trues.shape[-1])      # (4419072, 1)
