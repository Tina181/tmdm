...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        pred_ns = pred.mean(axis=1)  # (8, 192, 7)
                        mae, mse, rmse, mape, mspe = metric(np.array(pred_ns), np.array(true))
                        maes.append(mae)
                        mses.append(mse)
                        rmses.append(rmse)
                        mapes.append(mape)
        ...
        np.save(folder_path + 'metrics.npy', np.array([total_mse, total_mae, total_rmse, total_mape, total_mspe]))
        return
